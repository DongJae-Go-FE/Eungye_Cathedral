...
export default function SearchBar({ handleSearch, isLoading }: SearchType) {
  ...
  const inputContainerStyle = `has-disabled:bg-gray-200 body1m flex h-12 w-full items-center justify-between rounded-sm border px-4 ${isFocus ? (isLoading ? "border-gray-200" : "border-black") : "border-gray-300"}`;
  ...
  return (
    <form className={formStyle}>
      <label htmlFor="search" className="heading03b mb-3 block">
        검색
      </label>
      <div className="flex gap-x-4">
        <div className={inputContainerStyle}>
          ...
          <input
            ...
            name="search"
            id="search"
            className="h-full w-[calc(100%-30px)] text-black placeholder:text-gray-300 focus:outline-0 disabled:bg-gray-200"
            disabled={isLoading}
        ...
        <div className={btnContainerStyle}>
          <button
            type="button"
            className="mobile:hidden tablet:flex h-full w-[62px] cursor-pointer items-center gap-x-1 disabled:cursor-auto disabled:text-gray-300"
            disabled={isLoading}
            onClick={handleReset}
          >
            <svg
